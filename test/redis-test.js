            ...
            if (--todo === 0) {
                redis.createClient().multi([
                    ...
                    ['zrange', 'requests:response:end', '0', '-1',
                        'WITHSCORES'],
                    ['get', 'request:1:request:url'],
                    ['get', 'request:1:request:start'],
                    ['get', 'request:1:request:end'],
                    ['get', 'request:1:request:data'],
                    ['hgetall', 'request:1:request:headers'],
                    ['get', 'request:1:response:status'],
                    ['get', 'request:1:response:start'],
                    ['get', 'request:1:response:end'],
                    ['get', 'request:1:response:data'],
                    ['hgetall', 'request:1:response:headers'],
                    ['get', 'request:2:request:url'],
                    ['get', 'request:2:request:start'],
                    ['get', 'request:2:request:end'],
                    ['get', 'request:2:request:data'],
                    ['hgetall', 'request:2:request:headers'],
                    ['get', 'request:2:response:status'],
                    ['get', 'request:2:response:start'],
                    ['get', 'request:2:response:end'],
                    ['get', 'request:2:response:data'],
                    ['hgetall', 'request:2:response:headers']
                ]).exec(function(err, replies) {
                    var now = Date.now();
                    [0, 1, 2, 3].forEach(function(k) {
                        expect(replies[k].length).to.be(4);
                        expect(replies[k][0]).to.be('1');
                        expect(replies[k][2]).to.be('2');
                        expect(replies[k][1]).to.be.within(start, now);
                        expect(replies[k][3]).to.be.within(start, now);
                    });
                    expect([
                        'http://127.0.0.1:8082/url1',
                        'http://127.0.0.1:8082/url2']).to.contain(replies[4]);
                    expect([
                        'http://127.0.0.1:8082/url1',
                        'http://127.0.0.1:8082/url2']).to.contain(replies[14]);
                    ...
                    expect(['yes1', 'yes2']).to.contain(replies[13]['x-test']);
                    expect(['yes1', 'yes2']).to.contain(replies[23]['x-test']);
